...
\subsubsection{\bf \emph{SoftPLL} has crashed/restarted}
		...
		\begin{pck_descr}
			\item [] \underline{Status}: TODO \emph{(depends on SoftPLL mem read), (requires changes in RISC-V software)}
			\item [] \underline{Severity}: ERROR
			\item [] \underline{Mode}: \emph{all}
			\item [] \underline{Description}:\\
				If the RISC-V software crashes or restarts for some reason, its state may
			...
			\item [] \underline{Note}: We have a similar mechanism as in the
				\emph{wrpc-sw} to detect if the RISC-V program has restarted because of
				the CPU following a NULL pointer. However, RISC-V program hangs on
...
\subsubsection{\bf System/WR time has diverged from the reference time (NTP, NMEA, IRIG-B)}
		...
		\begin{pck_descr}
			...
			\item [] \underline{Description}:\\
				...
				threshold from the reference Time of Day acquired via
				NTP, NMEA or IRIG-B.
			\item [] \underline{SNMP objects}:\\
				{\footnotesize
				...
				\snmpadd{WR-SWITCH-MIB::wrsTimeOfDaySource}}
		\end{pck_descr}
		...
		\begin{pck_descr}
			...
			\item [] \underline{Description}:\\
				...
				\begin{itemize}
					...
					\item status of programming FPGA and SoftCPU
			...
			\item [] \underline{SNMP objects}:\\
				...
				\snmpadd{WR-SWITCH-MIB::wrsBootLoadSoftCPU}\\
...
%
%\subsection{SoftCPU software has crashed/restarted}
%
