...
\snmpentrys{WR-SWITCH-MIB}{}{wrsDetailedStatusesGroup}{
  ...
  \snmpentrys{WR-SWITCH-MIB}{wrsOSStatusGroup}{wrsBootSuccessful}{
    ...
    to load the RISC-V software, any kernel modules or userspace daemons are
  ...
  \snmpentrys{WR-SWITCH-MIB}{wrsTimingStatusGroup}{wrsSystemClockStatus}{
    ...
    clock diverged from the configured reference \textit{Time of Day} more than
    the configured
    ...
    \texttt{\glshyperlink{WR-SWITCH-MIB::wrsTimeOfDaySource}}.\\
    ...
    \begin{pck_proc}
      \item Check Syslog message to find one of the messages similar to:\\
        \texttt{SNMP: Error wrsSystemClockStatusDetails} \\
        \texttt{SNMP: Error wrsLeapSecStatus} \\
        \texttt{SNMP: Error wrsLeapSecStatusDetails} \\
        \texttt{SNMP: Error wrsLeapSecSourceStatus}  \\
        \texttt{SNMP: Error wrsLeapSecSourceStatusDetails} \\
        \texttt{SNMP: Error wrsTimeOfDaySource} \\
        Check also messages with \textit{Warning} instead of \textit{Error}.
        It may help you to retrieve the configuration problem.
      \item If \textit{Time of Day} source is configured as NTP, check that your
        configured NTP server is reachable using the \texttt{ping} command in
        the switch.
      \item If \textit{Time of Day} source is configured as NMEA or IRIG-B,
        check that proper cable is connected
        (for NMEA to serial port in the back,
        for IRIG-B to PPS-in connector in the front)
        and verify with \texttt{wr\_date} console tool available on WRS.
  ...
  \snmpentrys{WR-SWITCH-MIB}{wrsVersionGroup}{wrsVersionSwVersion}{
    ...
    from the repository (information provided from \texttt{git describe} command
    at build time).}
...
\snmpentrye{WR-SWITCH-MIB}{}{wrsOperationStatus}{}
  ...
  Result of the comparison of system clock and the Time of Day source
  (NTP, NMEA, IRIG-B) to detect a drift.}
  ...
  \snmpentrye{WR-SWITCH-MIB}{wrsCurrentTimeGroup}{wrsSystemClockDriftUs}{Drift
    ...
    values}
  \snmpentrye{WR-SWITCH-MIB}{wrsCurrentTimeGroup}{wrsTimeOfDaySource}{
    Configured source of \textit{Time of Day}.}
  ...
  \snmpentrye{WR-SWITCH-MIB}{wrsBootStatusGroup}{wrsBootLoadSoftCPU}{}
...
%	
  ...
  \snmpentrye{WR-SWITCH-MIB}{wrsPortStatusTable}{wrsPortStatusSfpTemp.<n>}{
    Temperature in C reported by SFP.}
  \snmpentrye{WR-SWITCH-MIB}{wrsPortStatusTable}{wrsPortStatusSfpVcc.<n>}{
    Internally measured transceiver supply voltage in mV reported by SFP.}
  \snmpentrye{WR-SWITCH-MIB}{wrsPortStatusTable}{wrsPortStatusSfpTxBias.<n>}{
  ...
  \snmpentrye{WR-SWITCH-MIB}{wrsPortStatusTable}{wrsPortStatusSfpTempF.<n>}{
    Temperature in 0.1 C reported by SFP.}
  \snmpentrye{WR-SWITCH-MIB}{wrsPortStatusTable}{wrsPortStatusSfpVccF.<n>}{
    Internally measured transceiver supply voltage in 0.1 mV reported by SFP.}
  \snmpentrye{WR-SWITCH-MIB}{wrsPortStatusTable}{wrsPortStatusSfpTxPowerF.<n>}{
    Measured TX output power in 0.1 uW reported by SFP.}
  \snmpentrye{WR-SWITCH-MIB}{wrsPortStatusTable}{wrsPortStatusSfpRxPowerF.<n>}{
